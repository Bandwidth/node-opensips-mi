...
describe("The util module", function () {
	...
	describe("With the old response", function () {
		describe("has the correct contact and properties", function () {
			var response;
			var expectedResults;
			var results;
			before(function () {
			...
			it("with the old function", function () {
				fixtures.loadFixture("show-contact/valid.txt")
				.then(function (fixture) {
					response = Response.fromBuffer(fixture);
			...
			it("with the new function", function () {
				fixtures.loadFixture("show-contact/valid.txt")
				.then(function (fixture) {
					response = Response.fromBuffer(fixture);
					results = util.extractContactListUpdated(response);
					expect(results).to.deep.equal(expectedResults);
				})
				.catch(function (err) {
					console.log(err);
	...
	describe("With the new response", function () {
		describe("returned the correct contact and properties", function () {
			var resultOld;
			var resultNew;
			var expectedResult;
			var response;
			...
			it("with the old function", function () {
				resultOld = util.extractContactList(response);
				expect(resultOld).to.deep.equal(expectedResult);
			});
			it("with the new function", function () {
				resultNew = util.extractContactListUpdated(response);
